...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = input.zipWithIndex.foreach{
    ...
    case (h, i) => output(i) = max(h/i, output(i-1))
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    def helper(idx: Int, maxPrev: Float): Float = {
      if (idx == until) maxPrev
      else helper(idx+1, max(maxPrev, input(idx)/idx))
    }
    helper(from, 0f)
  }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    ...
    case Leaf(s, e, _) => downsweepSequential(input, output, startingAngle, s, e)
  ...
  def parLineOfSight(input: Array[Float], output: Array[Float],
    threshold: Int): Unit = downsweep(input, output, 1,
